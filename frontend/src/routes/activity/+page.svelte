    ...
    {#each data.item as d}
      <a
        ...
        onclick={() => {
          is_idea = true;
          current_name = d.name;
          current_description = d.description;
          current_detailed_activity = d.detailed_activity;
        }}
      >
        <BannerTitleDescription idea={d.name} purpose={d.description} />
      </a>
    {/each}
  </div>
  {#if is_idea}
    ...
    >
      <Activity
        title={current_name}
        overview={current_description}
        details={current_detailed_activity}
      />
    </div>
  {/if}
</div>
